...
angular.module("rzTable").directive('rzTable', ['resizeStorage', '$injector', '$parse', function(resizeStorage, $injector, $parse) {
    ...
    function bindEventToHandle(scope, table, handle, column) {
        if($(handle).parent().hasClass('colHouseAddress')){
            specialHandle = handle; 
            specialColumn = column;
        };
        // This event starts the dragging
        if ($(handle).parent().hasClass('colBedroom')) {
            $(handle).mousedown(function(event) {
            if (isFirstDrag) {
                resizer.onFirstDrag(column, specialHandle);
            ...
            if (resizer.intervene) {
                optional = resizer.intervene.selector(specialColumn);
            ...
            $(specialHandle).addClass(scope.options.handleClassActive || 'rz-handle-active');

            // Get mouse and column origin measurements
            var orgX = event.clientX;
            var orgWidth = $(specialColumn).width();

            // On every mouse move, calculate the new width
            listener = calculateWidthEvent(scope, specialColumn, orgX, orgWidth, optional)
            $(window).mousemove(listener)

            // Stop dragging as soon as the mouse is released
            $(window).one('mouseup', unbindEvent(scope, specialColumn, specialHandle))
        })
        }else{
            $(handle).mousedown(function(event) {
                if (isFirstDrag) {
